...
- **Rééquilibrage** — sous les positions : poids cible par ligne, plan d'ordres complet, besoin de liquidités et impact fiscal des ventes. Remplace l'ancien simulateur d'ordre, qui ne raisonnait que sur une ligne à la fois alors que vendre l'une modifie le poids de toutes les autres
...
- **Dividendes & TRI** — dans Performance : dividendes attendus confrontés aux dividendes réellement encaissés, croissance année par année, rendement sur prix de revient, et **taux de rendement interne par ligne** (le PRU dit ce qu'on a payé, le TRI dit ce que l'argent a rapporté compte tenu de quand il a été investi)
